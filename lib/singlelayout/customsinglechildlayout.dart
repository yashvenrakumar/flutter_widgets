  ...
  Widget build(BuildContext context) {
    ...
    return new CustomSingleChildLayout(
      ...
      child: new Transform(
        ...
        transform: new Matrix4.identity()..rotateZ(rads),
...
class RotatedWidgetLayoutDelegate extends SingleChildLayoutDelegate {
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    return new Offset(
        (width - childSize.width) / 2, (height - childSize.height) / 2);
  }
...
class FreeRotatedBox extends SingleChildRenderObjectWidget {
  ...
  void updateRenderObject(
      BuildContext context, RenderFreeRotatedBox renderObject) {
    renderObject.degrees = degrees;
  }
}

class RenderFreeRotatedBox extends RenderBox
    with RenderObjectWithChildMixin<RenderBox> {
  RenderFreeRotatedBox({@required double degrees, RenderBox child})
      : assert(degrees != null),
        _degrees = degrees {
    this.child = child;
  }
  ...
  void performLayout() {
    ...
    if (child != null) {
      ...
      final rads = _degreesToRadians(this.degrees);
      final layoutWidth = math.sin(rads) * child.size.height +
          math.cos(rads) * child.size.width;
      final layoutHeight = math.sin(rads) * child.size.width +
          math.cos(rads) * child.size.height;
  ...
  // 	return child.hitTest(result, Offset: MatrixUtils.transformPoint(inverse, position));
  // }
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: WidgetWithCodeView(
        ...
        codeLinkPrefix:
            'https://github.com/yashvenrakumar/flutter_widgets/blob/master/lib/singlelayout/customsinglechildlayout.dart',
      ),
    );
  }
}
